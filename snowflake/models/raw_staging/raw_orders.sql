...
LEFT JOIN {{ source('sample','LINEITEM') }} AS a
    ON a.L_ORDERKEY = b.O_ORDERKEY
LEFT JOIN {{ source('sample','CUSTOMER') }} AS c
    ON b.O_CUSTKEY  = c.C_CUSTKEY
LEFT JOIN {{ source('sample','NATION') }}  AS d
    ON c.C_NATIONKEY  = d.N_NATIONKEY
LEFT JOIN {{ source('sample','REGION') }}  AS e
    ON d.N_REGIONKEY  = e.R_REGIONKEY
LEFT JOIN {{ source('sample','PART') }}  AS g
    ON a.L_PARTKEY = g.P_PARTKEY
LEFT JOIN {{ source('sample','SUPPLIER') }}  AS h
    ON a.L_SUPPKEY = h.S_SUPPKEY
LEFT JOIN {{ source('sample','NATION') }}  AS j
    ON h.S_NATIONKEY = j.N_NATIONKEY
LEFT JOIN {{ source('sample','REGION') }}  AS k
